...
def classify(path_to_palmline_image):
    ...
    print(palmline_img.shape, "palmline_img shape ++")
    ...
    palmline_gray = cv2.cvtColor(palmline_img, cv2.COLOR_BGR2GRAY)  # Convert to grayscale
    palmline_binary = cv2.threshold(palmline_gray, 127, 255, cv2.THRESH_BINARY)[1]  # Binarize image
    skel_img = skeletonize(palmline_binary // 255)  # Skeletonize (output is boolean)
    skel_img_uint8 = (skel_img * 255).astype(np.uint8)  # Convert to uint8

    #cv2.imwrite('results/skel.jpg',skel_img)
    
    lines = group(skel_img_uint8)  # get candidate lines
